...
def _upload_csv(client: TestClient, csv_payload: str, filename: str = "sample.csv"):
    return client.post(
        "/ingestions/csv",
...
def test_csv_ingestion_and_event_listing(client: TestClient) -> None:
    sample_csv = (
    ...
    response = _upload_csv(client, sample_csv)
...
def test_rule_engine_flags_new_ip_for_established_account(client: TestClient) -> None:
    sample_csv = (
        "id,account,group,IP,url,port,vlan,switchIP,time,ret\n"
...
def test_user_risk_summary_ranks_accounts_by_alert_score(client: TestClient) -> None:
    sample_csv = (
        "id,account,group,IP,url,port,vlan,switchIP,time,ret\n"
    ...
    summaries_response = client.get("/risk/users")
...
def test_dashboard_overview_returns_security_summary_counts(client: TestClient) -> None:
    sample_csv = (
        "id,account,group,IP,url,port,vlan,switchIP,time,ret\n"
...
def test_csv_ingestion_marks_job_failed_for_invalid_port(client: TestClient) -> None:
    sample_csv = (
        "id,account,group,IP,url,port,vlan,switchIP,time,ret\n"
        "1,user@example.com,engineering,192.168.1.10,http://example.com,not-a-port,700,10.0.0.1,2021/6/16 7:56,0.1149\n"
    )

    response = _upload_csv(client, sample_csv, filename="bad.csv")
...
def test_csv_ingestion_rejects_missing_required_columns(client: TestClient) -> None:
    sample_csv = (
    ...
    response = _upload_csv(client, sample_csv, filename="missing-columns.csv")
    assert response.status_code == 400
...
def test_isolation_forest_detection_scores_events_and_creates_ml_alerts(client: TestClient) -> None:
    sample_csv = (
        "id,account,group,IP,url,port,vlan,switchIP,time,ret\n"
    ...
    ingest_response = _upload_csv(client, sample_csv)
    assert ingest_response.status_code == 200

    detect_response = client.post("/detections/isolation-forest")
...
def test_isolation_forest_detection_is_idempotent_for_alert_creation(client: TestClient) -> None:
    sample_csv = (
        "id,account,group,IP,url,port,vlan,switchIP,time,ret\n"
        "1,user@example.com,engineering,192.168.1.10,http://example.com,443,700,10.0.0.1,2021/6/16 08:30,0.1\n"
        "2,user@example.com,engineering,192.168.1.10,http://example.com,443,700,10.0.0.1,2021/6/17 08:30,0.1\n"
    ...
    first_run = client.post("/detections/isolation-forest")
    assert first_run.status_code == 200
    second_run = client.post("/detections/isolation-forest")
